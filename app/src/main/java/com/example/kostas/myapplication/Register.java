...
public class Register extends AppCompatActivity {
    ...
    public void sendPost(String User) {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    URL url = new URL("http://192.168.0.104:3000/api/Trader?$class=org.example.trading.Trader&tradeId="+User+"&userName="+User+"");
                    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                    conn.setRequestMethod("POST");
                    ...
                    jsonParam.put("$class", "org.example.trading.Trader");
                    jsonParam.put("tradeId",User);
                    jsonParam.put("userName",User);
                    jsonParam.put("wallet",0);
